...
class AbandonedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Convert to binary label: 1 for abandoned/turn-exit (%), 0 for non-abandoned
        label = 1 if tag == '%' else 0
...
class AbandonedClassifier(nn.Module):
    ...
    def classify_utterance(self, utterance, device="cuda", pre_cleaned=False):
        """Classify a single utterance as abandoned/turn-exit (1) or not (0)."""
        self.eval()
        with torch.no_grad():
...
def train_model(model, train_loader, val_loader, num_epochs=3, device="cuda", save_path="best_abandoned_model.pth"):
...
def train_and_save_classifier(save_path="best_abandoned_model.pth", num_epochs=3):
    """Train the abandoned classifier and save it for later use."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")

    print("\nLoading datasets...")
    train_dataset = AbandonedDataset(split="train")
    test_dataset = AbandonedDataset(split="test")
    ...
    print("\nInitializing model...")
    model = AbandonedClassifier().to(device)

    # Train model
    train_model(model, train_loader, test_loader, num_epochs=num_epochs, device=device, save_path=save_path)

def load_classifier(model_path="best_abandoned_model.pth"):
    """Load a trained abandoned classifier for inference."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    # Initialize and load the model
    model = AbandonedClassifier(model_path=model_path).to(device)
    model.eval()
