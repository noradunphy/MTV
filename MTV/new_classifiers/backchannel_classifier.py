...
class BackchannelClassifier(nn.Module):
...
def train_model(model, train_loader, val_loader, num_epochs=3, device="cuda", save_path="best_backchannel_model.pth"):
...
def train_and_save_classifier(save_path="best_backchannel_model.pth", num_epochs=3):
    """Train the backchannel classifier and save it for later use."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")

    print("\nLoading datasets...")
    train_dataset = BackchannelDataset(split="train")
    test_dataset = BackchannelDataset(split="test")
    ...
    print("\nInitializing model...")
    model = BackchannelClassifier().to(device)

    # Train model
    train_model(model, train_loader, test_loader, num_epochs=num_epochs, device=device, save_path=save_path)

def load_classifier(model_path="best_backchannel_model.pth"):
    """Load a trained backchannel classifier for inference."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    # Initialize and load the model
    model = BackchannelClassifier(model_path=model_path).to(device)
    model.eval()
