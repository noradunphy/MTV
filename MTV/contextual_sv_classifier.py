...
class ContextualSVDataset(Dataset):
    ...
    def _print_example(self, idx):
        ...
        # Check if it's a statement-opinion (sv)
        is_sv = tag.startswith('sv')
        ...
        print(f"Is Statement-Opinion: {is_sv}")
    ...
    def __getitem__(self, idx):
        ...
        # Convert to binary label: 1 for statement-opinion (sv), 0 for non-sv
        label = 1 if tag.startswith('sv') else 0
...
class ContextualSVClassifier(nn.Module):
    ...
    def classify_utterance(self, current_utterance, previous_utterance="", device="cuda", pre_cleaned=False):
        """
        Classify a single utterance as statement-opinion (1) or not (0), considering the previous turn.
...
def train_model(model, train_loader, val_loader, num_epochs=3, device="cuda", save_path="best_contextual_sv_model.pth"):
...
def train_and_save_classifier(save_path="best_contextual_sv_model.pth", num_epochs=3):
    """Train the contextual statement-opinion classifier and save it for later use."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")

    print("\nLoading datasets...")
    train_dataset = ContextualSVDataset(split="train", debug=True)
    val_dataset = ContextualSVDataset(split="validation", debug=True)
    test_dataset = ContextualSVDataset(split="test", debug=True)
    ...
    # Count statement-opinions in each split
    train_sv = sum(1 for item in train_dataset.dataset if train_dataset.dataset.features['damsl_act_tag'].names[item['damsl_act_tag']].startswith('sv'))
    val_sv = sum(1 for item in val_dataset.dataset if val_dataset.dataset.features['damsl_act_tag'].names[item['damsl_act_tag']].startswith('sv'))
    test_sv = sum(1 for item in test_dataset.dataset if test_dataset.dataset.features['damsl_act_tag'].names[item['damsl_act_tag']].startswith('sv'))
    
    print("\nStatement-Opinion Distribution:")
    print(f"Training set: {train_sv} statement-opinions ({train_sv/len(train_dataset)*100:.2f}%)")
    print(f"Validation set: {val_sv} statement-opinions ({val_sv/len(val_dataset)*100:.2f}%)")
    print(f"Test set: {test_sv} statement-opinions ({test_sv/len(test_dataset)*100:.2f}%)")
    ...
    print("\nInitializing model...")
    model = ContextualSVClassifier().to(device)

    # Train model
    train_model(model, train_loader, val_loader, num_epochs=num_epochs, device=device, save_path=save_path)

def load_classifier(model_path="best_contextual_sv_model.pth"):
    """Load a trained contextual statement-opinion classifier for inference."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    # Initialize and load the model
    model = ContextualSVClassifier(model_path=model_path).to(device)
    model.eval()
